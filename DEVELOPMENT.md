...
- S7: doctor dry-run/live layering; dry-run proves static contracts, while live
  ...
  S3 object read/write/delete via the cached `minio-client` image, and two-Job
  RWX behavior when a digest-pinned smoke image is available
...
- P0 skeletons remain dry-run/contract-only. p1-real offline install performs
  ...
  format. Live doctor then runs the S3 probe from cached `minio-client` and the
  RWX smoke from cached `rwx-smoke` after the JuiceFS PVC is `Bound`.
