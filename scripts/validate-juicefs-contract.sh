...
require_line "${secret_rendered}" "  namespace: ${substrate_namespace}" "rendered JuiceFS Secret namespace must match SUBSTRATE_NAMESPACE"
...
require_line "${storage_rendered}" "  csi.storage.k8s.io/provisioner-secret-namespace: ${substrate_namespace}" "rendered StorageClass provisioner secret namespace must match SUBSTRATE_NAMESPACE"
...
require_line "${storage_rendered}" "  csi.storage.k8s.io/node-publish-secret-namespace: ${substrate_namespace}" "rendered StorageClass node-publish secret namespace must match SUBSTRATE_NAMESPACE"
require_line "${storage_rendered}" "  name: ${pvc_name}" "rendered JuiceFS PVC metadata.name must match JUICEFS_PVC_NAME"
require_line "${storage_rendered}" "  namespace: ${app_namespace}" "rendered JuiceFS PVC namespace must match KUBE_NAMESPACE"
