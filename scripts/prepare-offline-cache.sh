...
delegates to scripts/download-online.sh to produce a cacheMode: p1-real cache.
...
Common overrides:
  ...
  RWX_SMOKE_SOURCE=docker.io/library/busybox:1.36.1
EOF_USAGE
}
...
write_artifact_lock() {
  ...
  local rwx_smoke_archive rwx_smoke_image rwx_smoke_sha
  ...
  IFS=$'\t' read -r rwx_smoke_archive rwx_smoke_image rwx_smoke_sha <<<"${rwx_smoke_record}"
...
RWX_SMOKE_ARCHIVE_URL=file://${rwx_smoke_archive}
...
RWX_SMOKE_SOURCE="${RWX_SMOKE_SOURCE:-docker.io/library/busybox:1.36.1}"
...
rwx_smoke_record="$(prepare_image_artifact "RWX_SMOKE_SOURCE" "${RWX_SMOKE_SOURCE}" "${ARTIFACTS_DIR_ABS}/images/oci/rwx-smoke.tar")"
